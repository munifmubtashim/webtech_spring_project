<?php
// Defensive coding: safeguard data containers to prevent "Undefined variable" crashes
$u  = $user ?? [];
$ub = $upcomingBooking ?? null;
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="container">
    ...
    <div class="card">
        ...
        <div class="booking-card">
            <h3><?php echo htmlspecialchars($ub['room_type_name'] ?? 'Room Reservation'); ?></h3>
            <p>🛏 Room: <?php echo htmlspecialchars($ub['room_number'] ?? '-'); ?></p>
            <p>📅 Check-in:  <?php echo htmlspecialchars($ub['checkin_date'] ?? '-'); ?></p>
            <p>📅 Check-out: <?php echo htmlspecialchars($ub['checkout_date'] ?? '-'); ?></p>
            <p>💰 Total: $<?php echo number_format($ub['total_price'] ?? 0, 2); ?></p>
            <span class="status"><?php echo htmlspecialchars($ub['status'] ?? 'Confirmed'); ?></span>
        </div>
    </div>
    <?php endif; ?>
        ...
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" name="nationality"
                       value="<?php echo htmlspecialchars($u['nationality'] ?? ''); ?>">
            </div>
            <div class="form-group">
                <label>Preferred Room Type</label>
                <select name="preferred_room_type_id">
                    <option value="">No preference</option>
                    <option value="1" <?php echo (($u['preferred_room_type_id'] ?? '') == 1) ? 'selected' : ''; ?>>Standard</option>
                    <option value="2" <?php echo (($u['preferred_room_type_id'] ?? '') == 2) ? 'selected' : ''; ?>>Deluxe</option>
                    <option value="3" <?php echo (($u['preferred_room_type_id'] ?? '') == 3) ? 'selected' : ''; ?>>Suite</option>
                </select>
            </div>
            <div class="form-group">
                <label>Special Requests</label>
                <textarea name="special_requests"><?php echo htmlspecialchars($u['special_requests'] ?? ''); ?></textarea>
            </div>
